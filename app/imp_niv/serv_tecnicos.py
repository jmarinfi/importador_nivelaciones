...
class DBMetroLima:
    ...
    def get_sensors_lista(self, id_lista):
        return self.db.session.execute(
            text("SELECT LS.ID_LISTA, LS.ID_SENSOR, S.NOM_SENSOR, S.ID_EXTERNO, S.DESCRIPCION, S.ID_SISTEMA, (SELECT H.LECTURA FROM HISTORICO H WHERE H.ID_SENSOR = S.ID_SENSOR ORDER BY H.FECHA_MEDIDA DESC LIMIT 1) AS ULT_LECT, (SELECT H.FECHA_MEDIDA FROM HISTORICO H WHERE H.ID_SENSOR = S.ID_SENSOR ORDER BY H.FECHA_MEDIDA DESC LIMIT 1) AS ULT_FECHA FROM LISTAS_SENSORES LS JOIN SENSOR S ON LS.ID_SENSOR = S.ID_SENSOR WHERE LS.ID_LISTA = 625 AND S.ACTIVO = 1 ORDER BY LS.ORDEN;"),
            {'id_lista': id_lista}
        ).fetchall()
